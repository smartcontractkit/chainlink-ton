...
export type FiredrillEntrypointConfig = {
  id: bigint
...
export function firedrillEntrypointConfigToCell(config: FiredrillEntrypointConfig): Cell {
  const ownableCell = beginCell()
...
export class FiredrillEntrypoint implements Contract {
  ...
  static createFromConfig(config: FiredrillEntrypointConfig, code: Cell, workchain = 0) {
  ...
  async getOnRamps(provider: ContractProvider): Promise<OnRamps> {
    ...
    const slice = destChainSelectors.beginParse()
    const count = slice.loadUint(32)
    const selectors: bigint[] = []
    for (let i = 0; i < count; i++) {
      selectors.push(slice.loadUintBig(64))
    }
  ...
  async getOffRamps(provider: ContractProvider): Promise<OffRamps> {
    ...
    // Parse the selectors from the cell
    const slice = sourceChainSelectors.beginParse()
    const count = slice.loadUint(32)
    const selectors: bigint[] = []
    for (let i = 0; i < count; i++) {
      selectors.push(slice.loadUintBig(64))
  ...
  async getTokenPrice(provider: ContractProvider, token: Address): Promise<TimestampedPrice> {
    const result = await provider.get('tokenPrice', [{ type: 'slice', cell: beginCell().storeAddress(token).endCell() }])
    return {
