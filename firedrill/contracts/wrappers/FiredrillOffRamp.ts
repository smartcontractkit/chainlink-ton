...
export class FiredrillOffRamp implements Contract {
    ...
    async getSourceChainConfig(provider: ContractProvider, sourceChainSelector: bigint): Promise<SourceChainConfig> {
        const result = await provider.get('sourceChainConfig', [{ type: 'int', value: sourceChainSelector }]);
