...
describe('FiredrillEntrypoint - Unit Tests', () => {
    ...
    it('should allow owner to drill pending commit', async () => {
        ...
        const result = await entrypoint.sendDrillPendingCommitPendingQueueTxSpike(deployer.getSender(), {
            value: toNano('1.0'),
            from,
            to,
        });
        ...
        const onrampTransactions = result.transactions.filter(
            (tx) => tx.inMessage?.info.type === 'internal' && tx.inMessage.info.dest.equals(onramp.address),
        );
    ...
    it('should execute complete firedrill flow', async () => {
        ...
        const sendResult = await entrypoint.sendDrillPendingCommitPendingQueueTxSpike(deployer.getSender(), {
            value: toNano('1.0'),
            from: 1n,
            to: 3n,
        });
        ...
        const priceResult = await entrypoint.sendDrillPriceRegistries(deployer.getSender(), toNano('0.5'));
