...
import (
	...
	"integration-tests/trace_tracking/async/wrappers/two_phase_commit"
...
func TestTwoPhaseCommit(t *testing.T) {
	t.Run("AutoAck", func(t *testing.T) {
		...
		assert.NoError(t, err, "Failed to deploy counter A contract: %w", err)
		...
		assert.NoError(t, err, "Failed to deploy DB contract: %w", err)
		...
		assert.NoError(t, err, "Failed to begin transaction: %w", err)
		...
		assert.NoError(t, err, "Failed to get value from counter B: %w", err)
