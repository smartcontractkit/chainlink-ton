...
var INVENTORY_CONTRACT_PATH = test_utils.GetBuildDir("examples.async-communication.request-reply-with-two-dependencies.Inventory.compiled.json")

type InventoryProvider struct {
	apiClient trace_tracking.SignedAPIClient
}

func NewInventoryProvider(apiClient trace_tracking.SignedAPIClient) *InventoryProvider {
...
func (p Inventory) SendAddItem(key uint8, priceAddr *address.Address, countAddr *address.Address) (msgReceived *trace_tracking.ReceivedMessage, err error) {
	queryId := rand.Uint64()
