...
func loadJettonWalletCode() (*cell.Cell, error) {
	jettonWalletPath := path.Join(jetton_common.PathToContracts, "JettonWallet.compiled.json")
...
func TestJettonAll(t *testing.T) {
	...
	setUpCommon := func(t *testing.T) commonSetup {
		...
		setup.jettonMinter = DeployMinter(t, &setup.deployer, minter.InitData{
	...
	type senderSetup struct {
		common commonSetup
		sender *jetton_testing_wrappers.Sender
	}

	setupSender := func(t *testing.T) *senderSetup {
		setup := setUpCommon(t)

		t.Logf("Deploying Sender contract\n")
		Sender, err := jetton_testing_wrappers.NewSenderProvider(setup.deployer).Deploy(t.Context(), jetton_testing_wrappers.SenderInitData{
			MasterAddress: setup.jettonMinter.Address,
			WalletCode:    setup.jettonWalletCode,
		})
		require.NoError(t, err, "failed to deploy Sender contract")
		t.Logf("Sender contract deployed at %s\n", Sender.Contract.Address.String())
		...
		senderJettonWallet, err := setup.jettonClient.GetJettonWallet(t.Context(), Sender.Contract.Address)
	...
	type onrampMockSetup struct {
		common     commonSetup
		Sender     *jetton_testing_wrappers.Sender
		onrampMock *jetton_testing_wrappers.OnrampMock
	}
	...
	type simpleJettonReceiverSetup struct {
		common         commonSetup
		Sender         *jetton_testing_wrappers.Sender
		simpleReceiver *jetton_testing_wrappers.SimpleJettonReceiver
	}
	...
	setupSimpleJettonReceiver := func(t *testing.T) *simpleJettonReceiverSetup {
		...
		simpleJettonReceiver, err := jetton_testing_wrappers.NewSimpleJettonReceiverProvider(setup.common.deployer).Deploy(t.Context(), jetton_testing_wrappers.SimpleJettonReceiverInitData{
			JettonClient: jetton_common.Client{
				MasterAddress: setup.common.jettonMinter.Address,
				WalletCode:    setup.common.jettonWalletCode,
			},
	...
	t.Run("TestJettonMasterChangeContent", func(t *testing.T) {
		...
		t.Logf("Change content message received: \n%s\n", debugger.DumpReceived(changeContentMsg))
	...
	t.Run("TestJettonSendFastAutodeployWallet", func(t *testing.T) {
		...
		debugger := debug.NewDebuggerTreeTrace(map[string]debug.TypeAndVersion{
			...
			receiver.String():                               {Type: "Receiver", Version: *semver.MustParse("0.0.1")},
		})
		t.Logf("Sender message received: \n%s\n", debugger.DumpReceived(msgReceived))
	...
	t.Run("TestJettonSendFastExistingWallet", func(t *testing.T) {
		...
		t.Logf("Sender message received: \n%s\n", debugger2.DumpReceived(msgReceived))
	...
	t.Run("TestJettonOnrampMock", func(t *testing.T) {
		...
		sendCallWithAmount := func(jettonAmount tlb.Coins) (tracetracking.OutgoingExternalMessages, uint64, error) {
			...
			debugger := debug.NewDebuggerTreeTrace(map[string]debug.TypeAndVersion{
				...
				setup.common.jettonMinter.Address.String():      {Type: "JettonMinter", Version: *semver.MustParse("0.0.1")},
				setup.onrampMock.Contract.Address.String():      {Type: "OnrampMock", Version: *semver.MustParse("0.0.1")},
				onrampMockJettonWallet.Address().String():       {Type: "OnrampMockJettonWallet", Version: *semver.MustParse("0.0.1")},
			})
			t.Logf("Sender message received: \n%s\n", debugger.DumpReceived(msgReceived))
	...
	t.Run("TestJettonReceiver", func(t *testing.T) {
		...
		debugger := debug.NewDebuggerTreeTrace(map[string]debug.TypeAndVersion{
			...
			SenderWallet.Address().String():                 {Type: "SenderWallet", Version: *semver.MustParse("0.0.1")},
			setup.common.jettonMinter.Address.String():      {Type: "JettonMinter", Version: *semver.MustParse("0.0.1")},
			setup.simpleReceiver.Contract.Address.String():  {Type: "SimpleJettonReceiver", Version: *semver.MustParse("0.0.1")},
			SimpleJettonReceiverWallet.Address().String():   {Type: "SimpleJettonReceiverWallet", Version: *semver.MustParse("0.0.1")},
		})
	...
	t.Run("TestJettonTransferFromDeploy", func(t *testing.T) {
		...
		debugger = debug.NewDebuggerTreeTrace(map[string]debug.TypeAndVersion{
			...
			deployerJettonWallet.Address.String():           {Type: "JettonWallet", Version: *semver.MustParse("0.0.1")},
			setup.common.jettonMinter.Address.String():      {Type: "JettonMinter", Version: *semver.MustParse("0.0.1")},
			recipient.String():                              {Type: "Receiver", Version: *semver.MustParse("0.0.1")},
			receiverJettonWallet.Address().String():         {Type: "ReceiverJettonWallet", Version: *semver.MustParse("0.0.1")},
		})
		t.Logf("Transfer message received: \n%s\n", debugger.DumpReceived(transferMsg))
...
func DeployMinter(t *testing.T, deployer *tracetracking.SignedAPIClient, initData minter.InitData) *wrappers.Contract {
	...
	minterContract, deployMsg, err := wrappers.Deploy(t.Context(), deployer, minterCode, minterInitCell, tlb.MustFromTON("1"), topUpMsg)
	debugger := debug.NewDebuggerTreeTrace(map[string]debug.TypeAndVersion{
