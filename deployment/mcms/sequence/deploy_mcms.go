...
func deployMCMSSequence(b operations.Bundle, deps mcmsConfig.MCMSDeps, in DeployMCMSSeqInput) (DeployMCMSSeqOutput, error) {
	...
	if a.IsAddrNone() && (output.TimelockAddress == nil || output.TimelockAddress.TONAddress.IsAddrNone()) { // Deploy Timelock only if not deployed yet
		...
		body := timelock.Init{
			...
			Proposers:                common.SnakeData[common.WrappedAddress](common.WrapAddresses(in.ContractsParams.Timelock.Proposers)),
			Executors:                common.SnakeData[common.WrappedAddress](common.WrapAddresses(in.ContractsParams.Timelock.Executors)),
			Cancellers:               common.SnakeData[common.WrappedAddress](common.WrapAddresses(in.ContractsParams.Timelock.Cancellers)),
			Bypassers:                common.SnakeData[common.WrappedAddress](common.WrapAddresses(in.ContractsParams.Timelock.Bypassers)),
			ExecutorRoleCheckEnabled: true,
			OpFinalizationTimeout:    0,
		}
