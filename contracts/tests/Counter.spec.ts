...
describe('Counter', () => {
  ...
  it('should have type and version', async () => {
    const [type, version] = await bind.counter.getTypeAndVersion()
    expect(type.loadStringTail()).toBe('link.chain.ton.examples.Counter')
    expect(version.loadStringTail()).toBe(counter.CONTRACT_VERSION)
  })
