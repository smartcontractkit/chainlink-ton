...
async function setUpTest(i: bigint): Promise<{
    ...
    let responderCounter = blockchain.openContract(await ResponderCounterAdd.fromInit(0n, owner.address, i));
...
describe('ProxyUpgradeableCounter', () => {
    ...
    it('should be upgraded to version 2', async () => {
        ...
        let substractorCounter = await ResponderCounterSub.fromInit(0n, proxyCounter.address, 0n);
    ...
    //         getter,
    //     } = await setUpTest(3n);
    //     let substractorCounter = await ResponderCounterSub.fromInit(0n, owner.address, 0n);
    //     if (substractorCounter.init == null) {
    //         throw new Error('init is null');
    //     }
    //     let upgradeResult = await
