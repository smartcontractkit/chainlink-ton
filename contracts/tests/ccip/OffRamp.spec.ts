...
const versions = [
  {
    ...
    loadCode: async () => contractCode.ccip.release_1_6_0('ReceiveExecutor'),
  },
  {
    version: rx.RECEIVE_EXECUTOR_CONTRACT_VERSION,
    loadCode: async () => contractCode.ccip.local('ReceiveExecutor'),
  },
]
  ...
  (rxVersion) => {
    describe('OffRamp - Unit Tests', () => {
      ...
      const configDigest: bigint =
        0x0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcden
      ...
      beforeAll(async () => {
        ...
        receiveExecutorCodeRaw = await rxVersion.loadCode()
        ...
        _libs.set(
          BigInt(`0x${receiveExecutorCodeRaw.hash().toString('hex')}`),
      ...
      beforeEach(async () => {
        ...
        //
        {
          ...
          // setup ramp
          const updateRampsResult = await router.sendApplyRampUpdatesSetRamps(
            deployer.getSender(),
            {
              value: toNano('1'),
              data: {
      ...
      it('Test execute fails when different root was committed', async () => {
        ...
        const metadataHash = uint8ArrayToBigInt(getMetadataHash(CHAINSEL_EVM_TEST_90000001))
        const differentRootBytes = uint8ArrayToBigInt(
          generateMessageId(differentMessage, metadataHash),
        )
      ...
      afterAll(async () => {
        if (process.env['COVERAGE'] === 'true') {
          ...
          if (rxVersion.version == rx.RECEIVE_EXECUTOR_CONTRACT_VERSION) {
            contractList.push({
              code: await rxVersion.loadCode(),
              name: 'receive_executor',
            })
          }
          await coverage.generateCoverageArtifacts(blockchain, testSuitePrefix, contractList)
        }
      })
    })
  },
