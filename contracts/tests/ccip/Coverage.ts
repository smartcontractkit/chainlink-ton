...
export async function generateCoverageArtifacts(blockchain: Blockchain, testSuitePrefix: string, contracts: ContractCoverageConfig[]) {
    ...
    contracts.forEach(async (contract) => {
      ...
      writeFileSync(
        `./.coverage/${testSuitePrefix}_${contract.name}.json`,
        coverageJson
      )
    })
}
