...
export async function run(provider: NetworkProvider, args: string[]) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg.includes(':')) {
      ...
      try {
        ...
        throw new Error(`Invalid argument: ${arg}. Expected contract address or keyword argument (key:value)`)
      }
    }
  }
  ...
  for (let idx = 0; idx < contractAddresses.length; idx++) {
    ...
    console.log(`[${idx + 1}/${contractAddresses.length}] Processing ${contractAddress.toString()}...`)
...
async function withdrawFromContract(
  ...
  await withdrawable.sendWithdraw(
    contractProvider,
    sender,
    toNano('0.1'),
    withdrawParams,
