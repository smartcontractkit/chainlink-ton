...
export async function run(provider: NetworkProvider) {
  ...
  console.log(`📍 OffRamp: ${offRampAddress.toString()}\n`)
  ...
  // Step 1: Send drill pending commit (sends messages to OnRamp)
  console.log(`1️⃣  Sending drill pending commit (sequence ${fromSeq} to ${toSeq})...`)
  ...
  console.log('✅ Drill pending commit sent\n')

  // Step 2: Send drill pending execution (sends commit report to OffRamp)
  console.log('2️⃣  Sending drill pending execution...')

  await entrypoint.sendDrillPendingExecution(sender, {
    value: toNano('0.5'),
    from: fromSeq,
    to: toSeq,
  })
  ...
  await new Promise((resolve) => setTimeout(resolve, 3000))
  console.log('✅ Drill pending execution sent\n')
