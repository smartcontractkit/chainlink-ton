...
export function firedrillEntrypointConfigToCell(config: FiredrillEntrypointStorage): Cell {
    const ownable = beginCell().storeAddress(config.ownable.owner).storeAddress(config.ownable.pendingOwner);
...
export class FiredrillEntrypoint implements Contract {
    ...
    async sendInitRamps(provider: ContractProvider, via: Sender, value: bigint, onramp: Address, offramp: Address) {
        await provider.internal(via, {
            value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell().storeUint(Opcodes.initRamps, 32).storeAddress(onramp).storeAddress(offramp).endCell(),
        });
    }
    ...
    async getDestinationChainGasPrice(provider: ContractProvider, destChainSelector: bigint): Promise<GasPrice> {
        const result = await provider.get('destinationChainGasPrice', [{ type: 'int', value: destChainSelector }]);
