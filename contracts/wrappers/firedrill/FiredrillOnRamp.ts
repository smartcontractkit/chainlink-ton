...
export class FiredrillOnRamp implements Contract {
    ...
    async getDestChainConfig(provider: ContractProvider, destChainSelector: bigint): Promise<DestChainConfig> {
        const result = await provider.get('destChainConfig', [{ type: 'int', value: destChainSelector }]);
        return {
            ...
            allowedSenders:
                result.stack
                    .readCellOpt()
                    ?.beginParse()
                    .loadDictDirect(Dictionary.Keys.Address(), Dictionary.Values.Bool()) ?? Dictionary.empty(),
        };
    }
}
