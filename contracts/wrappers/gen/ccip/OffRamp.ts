...
export class OffRamp implements c.Contract {
    ...
    async getConfig(provider: ContractProvider): Promise<OffRamp_Config> {
        const r = StackReader.fromGetMethod(4, await provider.get('config', []));
        return ({
            $: 'OffRamp_Config',
            staticConfig: ({
                $: 'OffRamp_StaticConfig',
                rmnRouter: r.readSlice().loadAddress(),
                tokenAdminRegistry: r.readSlice().loadAddress(),
                chainSelector: r.readBigInt(),
            }),
            dynamicConfig: r.readCellRef<OffRamp_DynamicConfig>(OffRamp_DynamicConfig.fromSlice),
        });
    }
