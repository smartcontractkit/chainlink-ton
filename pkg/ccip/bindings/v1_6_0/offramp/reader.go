...
var GetOCR3Config = tvm.NewNoArgsGetter(tvm.NoArgsOpts[OCR3Base]{
	...
	Decoder: tvm.NewResultDecoder(func(r *ton.ExecutionResult) (OCR3Base, error) {
		...
		if !isNil {
			...
			commitSlice, err := configCell.BeginParse()
			if err != nil {
				return c, fmt.Errorf("begin parse OCR3Config cell: %w", err)
			}

			var config OCR3Config
			if err = tlb.LoadFromCell(&config, commitSlice); err != nil {
		...
		if !isNil {
			...
			executeSlice, err := configCell.BeginParse()
			if err != nil {
				return c, fmt.Errorf("begin parse OCR3Config cell: %w", err)
			}

			var config OCR3Config
			if err = tlb.LoadFromCell(&config, executeSlice); err != nil {
				return c, fmt.Errorf("load OCR3Config from cell: %w", err)
			}
			c.Execute = &config
		}
