...
func (a *TONAccessor) GetChainFeePriceUpdate(ctx context.Context, selectors []ccipocr3.ChainSelector) (map[ccipocr3.ChainSelector]ccipocr3.TimestampedUnixBig, error) {
	...
	for _, selector := range selectors {
		...
		packedValue, err := feequoter.PackGasPrice(gasPrice.ExecutionGasPrice, gasPrice.DataAvailabilityGasPrice)
		if err != nil {
			return nil, fmt.Errorf("failed to pack gas price for selector %d: %w", selector, err)
		}
