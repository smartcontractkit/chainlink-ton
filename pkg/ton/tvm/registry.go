...
// Fully Qualified Type is a identifier for a contract type following a reverse domain name notation.
//
...
//
// It maps contract types (FullyQualifiedType) to their corresponding TLBMap.
type ContractTLBRegistry map[FullyQualifiedType]TLBMap
...
func (r ContractTLBRegistry) Lookup(contract FullyQualifiedType, opcode uint64) (any, bool) {
	tlbs, ok := r[contract]
	if !ok {
		return nil, false
	}
