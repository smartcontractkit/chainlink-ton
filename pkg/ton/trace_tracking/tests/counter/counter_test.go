...
import (
	...
	counter "github.com/smartcontractkit/chainlink-ton/pkg/ton/trace_tracking/tests/counter/wrappers"
...
func TestCounter(t *testing.T) {
	t.Run("TestCounter", func(t *testing.T) {
		...
		assert.Equal(t, tvm.ExitCode_Success, msgReceived.ExitCode, "Expected exit code 0, got %d", msgReceived.ExitCode)
		...
		assert.Equal(t, tvm.ExitCode_Success, internalExitCode, "Expected exit code 0, got %d", internalExitCode)
