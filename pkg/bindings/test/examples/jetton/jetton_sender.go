...
func (p *SenderProvider) Deploy(ctx context.Context, initData SenderInitData) (*Sender, error) {
...
type SendJettonsFastMessage struct {
	_           tlb.Magic        `tlb:"#4C169F42" json:"-"` //nolint:revive // (opcode) should stay uninitialized
...
type SendJettonsExtendedMessage struct {
	_                tlb.Magic        `tlb:"#7FDA8110" json:"-"` //nolint:revive // (opcode) should stay uninitialized
	QueryID          uint64           `tlb:"## 64"`
	Amount           tlb.Coins        `tlb:"."`
	Destination      *address.Address `tlb:"addr"`
