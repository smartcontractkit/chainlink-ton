...
func NewCLDFOperationsEnvironment(bc []*blockchain.Input) ([]uint64, *deployment.Environment, error) {
	...
	rpcWSURL := bc[0].Out.Nodes[0].ExternalWSUrl
	rpcHTTPURL := bc[0].Out.Nodes[0].ExternalHTTPUrl
	...
	// TODO: add TON configuration for CLD here, does CLDF works with TON?
...
func deployContractsForSelector(in *Cfg, e *deployment.Environment, selector uint64) error {
	Plog.Info().Uint64("Selector", selector).Msg("Configuring per-chain contracts bundle")
	...
	// TODO: this is an example, add both EVM and TON changesets here
	...
	//in.TON.AddressesMu.Lock()
	//defer in.TON.AddressesMu.Unlock()
	//a, err := json.Marshal(addresses)
	//if err != nil {
	//	return err
	//}
	//in.TON.Addresses = append(in.TON.Addresses, string(a))
	return nil
}
...
func DefaultProductConfiguration(in *Cfg, phase ConfigPhase) error {
	...
	case ConfigureNodesNetwork:
		...
		evmBlockchain := in.Blockchains[0].Out.Nodes[0]
		evmBlockchainID := in.Blockchains[0].ChainID
...
`
	...
	case ConfigureProductContractsJobs:
		...
		transmittersSrc := make([]common.Address, 0)
		...
		// TODO: funding all CL nodes with TON
